document.addEventListener("DOMContentLoaded", function() {
    ...
    document.getElementById("login-form").addEventListener(
        "submit", function(event) {
            ...
            if (
                registeredUser &&
                JSON.parse(registeredUser).password === password
            ) {
    ...
    document.getElementById("register-form").addEventListener(
        "submit", function(event) {
            ...
            if (localStorage.getItem(email)) {
                ...
                localStorage.setItem("loggedInUser", email);
                displayAuthModal(
                    `Thank you for registering, ${email}! You can now log in.`,
                    "index.html"
                );
            }
        }
    ...
    document.getElementById("forgot-password-form").addEventListener(
        "submit", function(event) {
            event.preventDefault();
            ...
            if (localStorage.getItem(email)) {
                ...
                displayAuthModal(
                    "No account found with this email. Please try again or register."
                );
            }
    ...
    document.getElementById("forgot-password-link").addEventListener(
